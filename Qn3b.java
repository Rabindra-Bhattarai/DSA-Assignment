...
public class Qn3b extends JPanel implements ActionListener {
    ...
    private static final Color[] COLORS = { Color.RED, Color.GREEN, Color.BLUE, Color.YELLOW, Color.ORANGE }; // Colors
                                                                                                              // for the
                                                                                                              // blocks
    ...
    public Qn3b() {
        setPreferredSize(new Dimension(BOARD_WIDTH * BLOCK_SIZE, BOARD_HEIGHT * BLOCK_SIZE)); // Set the size of the
                                                                                              // game panel
        ...
        for (int row = 0; row < block.shape.length; row++) {
            for (int col = 0; col < block.shape[row].length; col++) {
                if (block.shape[row][col] != 0) { // Check if the current cell in the block is occupied
                    ...
                    if (newRow >= BOARD_HEIGHT || (newRow >= 0 && board[newRow][newCol] != 0)) { // Check if the new
                                                                                                 // position is out of
                                                                                                 // bounds or occupied
        ...
        for (int row = 0; row < block.shape.length; row++) {
            for (int col = 0; col < block.shape[row].length; col++) {
                if (block.shape[row][col] != 0) { // Check if the current cell in the block is occupied
                    ...
                    if (newCol < 0 || (newRow >= 0 && board[newRow][newCol] != 0)) { // Check if the new position is out
                                                                                     // of bounds or occupied
        ...
        for (int row = 0; row < block.shape.length; row++) {
            for (int col = 0; col < block.shape[row].length; col++) {
                if (block.shape[row][col] != 0) { // Check if the current cell in the block is occupied
                    int newRow = block.row + row; // Calculate the new row position
                    int newCol = block.col + col + 1; // Calculate the new column position
                    if (newCol >= BOARD_WIDTH || (newRow >= 0 && board[newRow][newCol] != 0)) { // Check if the new and
                                                                                                // position is out of
                                                                                                // and bounds or
                                                                                                // occupied
    ...
    public static void main(String[] args) {
        ...
        Qn3b game = new Qn3b(); // Create a new instance of the game
